...
<x-layouts.auth>
    <section>
        ...
        <form action="{{route('contacts.update', $contact)}}" method="post" class="flex flex-col gap-6 px-10 mx-auto" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="flex gap-6">
                <fieldset class="w-1/2 flex flex-col gap-5 border-r-gray-200 border-r pr-4">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="name" value="{{old('name')??$contact->name}}" name="name"
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="email" value="{{old('email') ?? $contact->email}}" name="email"
                               class="border border-gray-300 rounded-md block px-2 py-2 w-full"
                    ...
                    <div class="relative">
                        ...
                        <input type="file" id="avatar" value="{{old('avatar') ?? $contact->avatar}}" name="avatar"
                               class="border border-gray-300 rounded-md block px-2 py-2 w-full">
                        <small class="text-red-500 absolute -bottom-5">
                            @error('avatar')
                            {{ $message }}
                ...
                <fieldset class="w-1/2 flex flex-col gap-5">
                    ...
                    <div class="relative flex flex-col justify-around">
                        @if($attendances->count() !== 0 || $jiris->count() !== 0)
                            @foreach($attendances as $attendance)
                                <div class="flex gap-10 border-b py-5 border-b-gray-300 last-of-type:border-b-0">
                                    <div class="flex-1 flex items-center gap-5">
                                        <input type="checkbox" name="jiris[{{$jiri_attendance->id}}]"
                                               id="jiris[{{$jiri_attendance->id}}]" value="{{$jiri_attendance->id}}" checked>
                                        <label for="jiris[{{$jiri_attendance->id}}]" class="flex flex-col">
                                            {{$jiri_attendance->name}}
                                            <small class="text-xs">{{$jiri_attendance->description}}</small>
                                        </label>
                                    </div>
                                    <div class="flex flex-col">
                                        <label for="roles[{{$jiri_attendance->id}}]" class="text-right">
                                            Rôle
                                        </label>
                                        <select name="roles[{{$jiri_attendance->id}}]" id="roles[{{$jiri_attendance->id}}]"
                                                class="border-2 p-1 border-blue-400 rounded-lg">
                                            @if($attendance->role === ContactRoles::Evaluated->value)
                                                <option
